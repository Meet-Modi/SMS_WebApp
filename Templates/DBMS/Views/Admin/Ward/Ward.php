	...
	<header class="header1">
		...
		<div class="row">
			...
			<div class="block4 wrap-pic-w">
				<form action="Ward.php">
					<div class='w-size2 p-t-20'>
						<button class='flex-c-m size2 bg4 bo-rad-23 hov1 m-text3 trans-0-4' name="CheckWardStatus" value="CheckWardStatus">
							Check Ward Status.
						</button>
					</div>
				</form>
		...
		<?php 
			...
			if(isset($_GET['AddPatient'])){
				...
				<form action='Ward.php'>
					...
					<div class='effect1 w-size9'>
						Ward No :
						<input class='s-text7 bg6 w-full p-b-5' type='text' name='ward_no'>		
					</div>
					<br>
			...
			if(isset($_GET['PatientAdded'])){
				...
				$pid = $_GET["ID"];				
				$sql = "UPDATE patient SET Ward_No='$ward_no' WHERE Patient_ID='$pid'";
				echo $sql;
			...
			if(isset($_GET['CheckWardStatus'])){
					$sql = "CALL CheckWardStatus()";
					$result = mysqli_query($conn,$sql);
					echo "<br><center><p class='meet-head'>Ward Status</p><br>";
					if ($result->num_rows > 0) 
					{    
						...
						while ($row = $result->fetch_assoc()) 
						{ 
							echo "<tr><th>".$row["Ward_No"]."</th>
								  <th>".$row["Occupied"]."</th>
								  <th>".$row["Ward_Capacity"]."</th>
							      </tr>";
						}
								  echo "</table>";
					} else 
